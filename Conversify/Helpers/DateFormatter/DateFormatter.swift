...
public extension Date {
    ...
    init(fromString string: String, format:DateFormat, timeZone: TimeZone = .local)
    {
        ...
        case .iso8601(let isoFormat):
            ...
            let dateFormat = (isoFormat != nil) ? isoFormat! : Conversify.ISO8601Format(dateString: string as String)
    ...
    //    func differenceFromTodayInYears() -> String {
    //        let formatter = DateComponentsFormatter()
    //        formatter.allowedUnits = [.year]
    //        formatter.unitsStyle = .full
    //        let differenceOfDates = formatter.string(from: self, to: Date())
    //        //print(/differenceOfDates?.digits)
    //        return differenceOfDates?.digits ?? ""
    //    }
    ...
    func toString(_ format: DateFormat, timeZone: TimeZone = .local) -> String
    {
        ...
        case .iso8601(let isoFormat):
            dateFormat = (isoFormat != nil) ? isoFormat!.rawValue : Conversify.ISO8601Format.DateTimeMilliSec.rawValue
    ...
    func toTimePassedFormat() -> String {
        let date = Date()
        let calendar = Calendar.autoupdatingCurrent
        let components = (calendar as NSCalendar).components([.year, .month, .day, .hour, .minute, .second], from: self, to: date, options: [])
        if components.hour! >= 1 {
            return self.toString(format: "MMM d, yyyy")
        }else {
            return self.toString(format: "E h:mm a")
        }
    }
}
