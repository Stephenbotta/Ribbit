...
final class SMRippleView: UIView {
    ...
    fileprivate func commonInit(_ frame: CGRect, rippleColor: UIColor = UIColor.black, rippleThickness: Float = 0.5, rippleTimer: Float = 1,  fillColor: UIColor?, animationDuration: Double, parentFrame: CGRect) {
        ...
        self.rippleEndScale = Float(maxSize) / Float(self.frame.width)
    ...
    private func continuousRipples() {
        ...
        animation.setValue(layer, forKey: "item")
...
// extension for removing layer from super layer after animation completes

extension SMRippleView: CAAnimationDelegate {
    
    func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        if let layer = anim.value(forKey: "item") as? CALayer {
