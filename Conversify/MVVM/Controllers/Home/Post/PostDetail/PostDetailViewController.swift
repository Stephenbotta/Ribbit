///Users/admin/Documents/Chanchal/ribbit_ios/Podfile
...
typealias SelectPostDetail = (PostList) -> ()

class PostDetailViewController: BaseRxViewController {
    ...
    override func bindings() {
        ...
        btnInfo.rx.tap.asDriver().drive(onNext: { [unowned self] () in
            ...
            if /self.postDetailViewModal.postDetail.value?.postBy?.id != /Singleton.sharedInstance.loggedInUser?.id {
                return
            }
            
            UtilityFunctions.show(nativeActionSheet: "Select action", subTitle: "", vc: self, senders: self.postDetailViewModal.postDetail.value?.postType == "REGULAR" ?   ["Delete post" , "Edit post"] : ["Delete post"], success: { [weak self] (value, index) in
                self?.view.endEditing(true)
                switch index {
                
                case 0:
                    self?.postDetailViewModal.deletePost(postId: /self?.postDetailViewModal.postDetail.value?.id , { (status) in
                        self?.refresh?()
                        self?.popVC()
                    })
                default:
                    guard let vc = R.storyboard.post.createPostViewController() else {  return }
                    vc.createPostModal = CreatePostViewModal(postInfo: self?.postDetailViewModal.postDetail.value)
                    self?.pushVC(vc)
                }
            })
        })<bag
...
extension PostDetailViewController {
    ...
    func calcHeight(){
        let height1 = 120 + labelPostText.getEstimatedHeight()
...
extension PostDetailViewController : UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if /self.postDetailViewModal.arrayOfComments.value[/indexPath.section].header?.isHide {
            ...
            //            guard let cell = tableView.cellForRow(at: indexPath) as? PostDetailTableCell else { return 0.0 }
            //            /let cmntHeight = cell.labelName.getEstimatedHeight() + 16 + 4 + 24 + 14
...
extension PostDetailViewController : DelegateCommentHeaderView {
    ...
    func deleteComment(indx : Int?){
        ...
        postDetailViewModal.deleteCommentReply(mediaId: "", commentId: /postDetailViewModal.arrayOfComments.value[/indx].header?.id, replyId: "")
    ...
    func likeOnComment(indx: Int?) {
        ...
        postDetailViewModal.likeUnlikeComment(mediaId: "", commentId: /postDetailViewModal.arrayOfComments.value[/indx].header?.id, action: action, commentBy: /postDetailViewModal.arrayOfComments.value[/indx].header?.commentBy?.id) { (status) in
...
//        //update keyboard and scroll row near to keyboard
    ...
    func loadMoreReplies(indx: Int? , _ completion:@escaping (Int)->()) {
        ...
        postDetailViewModal.getCommentReplies(mediaId: "", indx: indx, commentId: postDetailViewModal.arrayOfComments.value[/indx].header?.id, replyId: replyId, totalReply: postDetailViewModal.arrayOfComments.value[/indx].header?.replyCount?.toString) { [weak self](isSuccess) in
            if isSuccess {
...
extension PostDetailViewController : DelegateCommentAccessory {
    ...
    func sendComment(cmnt: String?, attachmentUrl: NSDictionary) {
        ...
        if selectedComment != nil && !(/cmnt?.isEmpty)  {
            ...
            postDetailViewModal.replyOnComment(mediaId: "", sec : /copiedSelectedIndx ,commentBy: copiedSelectedComment?.commentBy?.id, commentId: /copiedSelectedComment?.id, userIdTag: (userTags?.count == 0) ? nil : userTags?.toJson(), replyId: nil, reply: cmnt) { (_) in
            }
            self.tableView.inputAccessory.constraintHeightViewReply.constant = 0
            tableView.inputAccessory.btnCamera.isHidden = false
        }else {
            print("api hit comment")
            postDetailViewModal.commentOnPost(mediaId: "", postId: postDetailViewModal.postDetail.value?.id, commentId: nil, userHashTag: (userTags?.count == 0) ? nil : userTags?.toJson(), comment: cmnt, postBy:  postDetailViewModal.postDetail.value?.postBy?.id, attachmentUrl: attachmentUrl) { [weak self](_) in
    ...
    func likePost() {
        ...
        postListModal.likePost(mediaId: "", postId: /postDetailViewModal.postId.value, action: action, postBy: postDetailViewModal.postDetail.value?.postBy?.id , { [weak self](status) in
            if status {
...
extension PostDetailViewController : DelegateRepliesCell {
    ...
    func deleteReply(indx : IndexPath?){
        ...
        postDetailViewModal.deleteCommentReply(mediaId: "", commentId: "", replyId: /postDetailViewModal.arrayOfComments.value[/indx?.section].items[/indx?.row].id)
        ez.runThisInMainThread { [weak self] in
    ...
    func likeReply(indx: IndexPath?) {
        ...
        postDetailViewModal.likeUnlikeReply(mediaId: "", replyId: item.items[/indx?.row].id, action: action, replyBy: item.items[/indx?.row].replyBy?.id) { (_) in
        }
    }
...
extension PostDetailViewController : DelegateUserSerachTag {
