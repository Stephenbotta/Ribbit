...
class TwitterCommentVC: BaseRxViewController {
    ...
    func callToPostDetailApi(){
        ...
        postDetailViewModal.getPostDetail { [weak self](status) in
            ...
            if status {
               ...
               // self?.imgPost.image(url:  /self?.postDetailViewModal.postDetail.value?.media?.first?.original), placeholderImage: #imageLiteral(resourceName: "ic_placeholder"))//kf.setImage(with: URL(string: /self?.postDetailViewModal.postDetail.value?.media?.first?.original))
...
extension TwitterCommentVC : UITableViewDelegate {
...
extension TwitterCommentVC : DelegateCommentHeaderView {
    ...
    func replyOnComment(indx: Int?) {
        ...
        tableView.inputAccessory.constraintHeightViewReply.constant = 48
...
extension TwitterCommentVC : DelegateCommentAccessory {
...
extension TwitterCommentVC : DelegateRepliesCell {
...
extension TwitterCommentVC : DelegateUserSerachTag {
...
extension TwitterCommentVC: UICollectionViewDataSource  , UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        //        UIApplication.topViewController()?.pushVC(vc)
        self.view.endEditing(true)
        guard let media = postDetailViewModal.postDetail.value?.media?[indexPath.row] else { return }
