package com.conversify.utils.video;
...
public class MediaController {
    ...
    private long readAndWriteTrack(MediaExtractor extractor, MP4Builder mediaMuxer, MediaCodec.BufferInfo info, long start, long end, File file, boolean isAudio) throws Exception {
        int trackIndex = findTrack(extractor, isAudio);
        if (trackIndex >= 0) {
    ...
    public File convertVideo(@NonNull final String videoPath, @NonNull final File cacheDir,
        ...
        if (resultWidth != 0 && resultHeight != 0) {
            ...
            try {
                    ...
                    if (videoIndex >= 0) {
                        ...
                        try {
                            ...
                            if (Build.VERSION.SDK_INT < 18) {
                                ...
                                Timber.i("codec = " + codecInfo.getName() + " manufacturer = " + manufacturer + "device = " + Build.MODEL);
                            } else {
                                colorFormat = MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface;
                            }
                            Timber.i("colorFormat = %s", colorFormat);
                                ...
                                while (decoderOutputAvailable || encoderOutputAvailable) {
                                    ...
                                    if (!decoderDone) {
                                        ...
                                        if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                                            ...
                                            Timber.i("newFormat = %s", newFormat);
                                            ...
                                            if (startTime > 0 && videoTime == -1) {
                                                if (info.presentationTimeUs < startTime) {
                                                    doRender = false;
                                                    Timber.i("drop frame startTime = " + startTime + " present time = " + info.presentationTimeUs);
                                                } else {
                                                ...
                                                if (!errorWait) {
                                                    if (Build.VERSION.SDK_INT >= 18) {
                                                        ...
                                                        if (inputBufIndex >= 0) {
                                                            ...
                                                            Timber.w("input buffer not available");
                                                        }
                                                    }
                                                }
                                            }
                ...
                Timber.i("Compression time = %s", (System.currentTimeMillis() - time));
            }
        } else {
            didWriteData(true, true);
            return null;
