...
const sendError = function (data) {
    ...
    if (typeof data === 'object' && data.hasOwnProperty('statusCode') && data.hasOwnProperty('customMessage')) {
        data.customMessage = __(data.customMessage);
        ...
        customErrorMessage = __(customErrorMessage);
...
const generateAccountString = function () {
    let options = {
        length:6,
        charset:'alphabetic',
